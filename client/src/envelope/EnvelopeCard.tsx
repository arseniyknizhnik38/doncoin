...
export function EnvelopeCard({ api }: { api: EnvelopeApi }) {
  ...
  if (api.justOpened) {
    return (
      <button
        type="button"
        onClick={api.dismiss}
        className="w-full truncate rounded-xl border border-don-gold/60 bg-don-ink px-3 min-h-11 inline-flex items-center justify-center py-1.5 text-center text-sm font-semibold text-don-gold-soft active:scale-95"
      >
  ...
  if (!envelope.unlocked) {
    return (
      <p className="truncate rounded-xl border border-dashed border-don-blood/40 px-3 py-1 text-center text-[11px] tracking-wider text-neutral-400">
  ...
  return (
    <button
      ...
      onClick={api.open}
      className="w-full rounded-xl bg-gradient-to-r from-don-blood to-don-blood-deep px-3 min-h-11 inline-flex items-center justify-center py-2 text-sm font-semibold text-don-gold-soft active:scale-95 disabled:opacity-50"
    >
